          ...
          {PROCESO_CONTENT.steps.map((paso, i) => {
            ...
            return (
              <motion.div key={i} variants={scaleUp} className="group relative flex flex-col items-center text-center">
                {/* Glow del paso */}
                <div className="absolute top-4 h-16 w-16 rounded-full bg-red-400/20 blur-xl transition-all duration-500 group-hover:bg-red-500/40 group-hover:blur-2xl" />
                
                <div
                  className={`relative mb-8 flex h-[4.5rem] w-[4.5rem] items-center justify-center rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50 transition-all duration-500 group-hover:-translate-y-2 group-hover:border-red-500/30 group-hover:shadow-2xl group-hover:shadow-red-500/20`}
                >
                  <div
                  ...
                  <span className="absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-lg bg-[#cc1b1b] text-[10px] font-black text-white shadow-sm ring-2 ring-white transition-transform duration-500 group-hover:rotate-12">
                    {i + 1}
                  </span>
                </div>
