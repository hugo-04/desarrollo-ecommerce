...
export default function ProductoPage() {
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-7xl px-4 py-3">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 text-xs min-w-0">
              ...
              <IconChevronRight className="h-3 w-3 shrink-0 text-slate-400" />
              <Link href={`/catalogo?categoria=${encodeURIComponent(product.category)}`} className="shrink-0 text-slate-500 hover:text-primary">{product.category}</Link>
              <IconChevronRight className="h-3 w-3 shrink-0 text-slate-400" />
      ...
      {/* Quote Modal */}
      {showQuoteModal && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center" onClick={() => setShowQuoteModal(false)}>
          ...
          <div className="relative w-full max-w-md rounded-t-3xl bg-white p-6 shadow-2xl sm:rounded-3xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowQuoteModal(false)} className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200">
              <IconX className="h-4 w-4" />
            </button>
            <div className="mb-5">
              <div className="mb-1 flex items-center gap-2">
                <IconWhatsApp className="h-5 w-5 text-green-600" />
                <p className="text-xs font-bold uppercase tracking-wider text-green-700">Solicitar Cotización</p>
              </div>
            ...
            <div className="mb-4">
              <label className="mb-1.5 block text-xs font-semibold text-slate-700">¿Alguna nota o cantidad específica? (opcional)</label>
              <textarea
                rows={3}
                value={quoteNote}
                onChange={(e) => setQuoteNote(e.target.value)}
                placeholder="Ej: Necesito 50 unidades para proyecto de subestación..."
            ...
            <button
              onClick={() => handleWhatsAppQuote(quoteNote)}
            ...
            <p className="mt-3 text-center text-[10px] text-slate-400">Respuesta en menos de 24 horas · L-V 9am-6pm · Sáb 9am-3pm</p>
          </div>
        </div>
      )}
