...
import { getEntidadesBancariasPagedAction, deleteEntidadBancariaAction } from "@/features/entidades-bancarias/actions"
...
import { entidadBancariaKeys }     from "@/features/entidades-bancarias/hooks"
...
function EntidadRow({ ent, index, removingId, onDelete }: {
  ent: EntidadBancariaDTO; index: number; removingId: number | null; onDelete: () => void
}) {
  const isRemoving = removingId === ent.id
  return (
    <TableRow
    ...
    >
      ...
      {/* Nombre */}
      <TableCell className="px-4 py-3 font-semibold text-slate-800">{ent.name}</TableCell>

      {/* Orden */}
      <TableCell className="px-4 py-3 text-sm text-slate-500 tabular-nums">{ent.order}</TableCell>
      ...
      {/* Fecha */}
      <TableCell className="px-4 py-3 text-xs text-slate-500 tabular-nums">{fmtDate(ent.updatedAt)}</TableCell>

      {/* Acciones */}
      <TableCell className="px-4 py-3">
        <div className="flex gap-2">
          <Button asChild variant="outline" size="xs">
            <Link href={`/entidades-bancarias/${ent.id}/editar`}>Editar</Link>
          </Button>
          <DeleteDialog
            trigger={
...
export function AdminEntidadesBancariasView() {
  const {
    ...
    queryKey: entidadBancariaKeys.lists(),
  })
  ...
  return (
    <div>
      <AdminListHeader
        ...
        subtitle={loading || fetching ? "Cargando…" : `${total} entidad${total !== 1 ? "es" : ""}`}
        newHref="/entidades-bancarias/nueva"
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow className="border-slate-100">
                <TableHead className="w-20 px-4 py-3 font-semibold text-slate-600">Logo</TableHead>
              ...
              {!loading && !fetching && items.length === 0 && (
                <TableRow>
                  <TableCell colSpan={6} className="py-10 text-center text-slate-400">
                    {search ? "Sin resultados para la búsqueda." : "No hay entidades bancarias aún."}
                  </TableCell>
                </TableRow>
              )}
              {!loading && !fetching && items.map((ent, i) => (
                <EntidadRow
                  key={`${currentPage}-${ent.id}`}
                  ent={ent} index={i} removingId={removingId}
                  onDelete={() => handleDelete(ent.id, () => deleteEntidadBancariaAction(ent.id), ent.name)}
                />
              ))}
            </TableBody>
          </Table>
